...
//!
//! BTG mirrors BCH/BSV/LTC: Trezor Blockbook REST surface (`/api/v2/...`)
//! for balance, UTXOs, history, fee estimate, broadcast, and tx-status.
//! Public Trezor instance: `https://btg1.trezor.io`.
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BtgSendResult {
    pub txid: String,
    #[serde(default)]
    pub raw_tx_hex: String,
}

impl super::SignedSubmission for BtgSendResult {
...
impl BitcoinGoldClient {
    ...
    pub async fn fetch_balance(&self, address: &str) -> Result<BtgBalance, String> {
        ...
        Ok(BtgBalance {
            balance_sat: sat,
            balance_display: format_btg(sat),
        })
    }

    pub async fn fetch_utxos(&self, address: &str) -> Result<Vec<BtgUtxo>, String> {
    ...
    pub async fn fetch_fee_rate(&self, blocks: u32) -> u64 {
        ...
        estimate
            ...
            .map(|btg_per_kb| ((btg_per_kb * 1e8 / 1000.0).ceil() as u64).max(1))
            .unwrap_or(1)
    }

    pub async fn fetch_history(&self, address: &str) -> Result<Vec<BtgHistoryEntry>, String> {
