...
impl WalletService {
    ...
    /// Drop everything a chain derived on the network it is leaving: the
    /// keypool's reserved indices and the discovered owned addresses.
    ///
    /// The single call to make on a network switch, for the same reason
    /// `delete_wallet_relational_data` is the single call to make on a wallet
    /// delete. The two deletes used to be issued separately by the caller,
    /// which could leave one side behind when the second failed: a keypool
    /// that outlives its network hands out indices for addresses the new one
    /// never derived, and owned addresses that outlive it are attributed to a
    /// network they were not derived on. Both tables now go in one
    /// transaction, and the in-memory copies go with them.
    pub async fn reset_chain_derivation_state(
        &self,
        chain_name: String,
    ) -> Result<(), SpectraBridgeError> {
        self.write_persisted(move |service| async move {
            let stored = service.state_db_path.read().await.clone();
            if let Some(db_path) = stored {
                let to_delete = chain_name.clone();
                tokio::task::spawn_blocking(move || {
                    crate::store::wallet_db::chain_derivation_delete_for_chain(&db_path, &to_delete)
                })
                .await
                .map_err(|e| SpectraBridgeError::from(format!("spawn_blocking: {e}")))?
                .map_err(SpectraBridgeError::from)?;
            }
            // The in-memory copies are the same state by another name, so they
            // only drop once the stored rows have.
            {
