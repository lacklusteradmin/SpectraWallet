...
private extension ImportedWallet {
    func makeAddressSnapshots() -> [PlatformWalletAddressSnapshot] {
        ...
        return candidates.compactMap { chainName, address in
            guard let resolvedAddress = address?.platformTrimmedOrNil,
                  let chainID = WalletChainID(chainName) else {
                return nil
            }
            return PlatformWalletAddressSnapshot(
                id: "\(id.uuidString):\(chainID.rawValue)",
...
extension Coin: PlatformSnapshotConvertible {
    func makePlatformSnapshot() -> PlatformAssetSnapshot {
        ...
        return PlatformAssetSnapshot(
            ...
            contractAddress: contractAddress?.platformTrimmedOrNil,
...
extension ImportedWallet: PlatformSnapshotConvertible {
    func makePlatformSnapshot() -> PlatformWalletSnapshot {
        ...
        return PlatformWalletSnapshot(
            id: id,
            name: name,
            selectedChainID: chainID.rawValue,
            selectedChainName: chainID.displayName,
            includeInPortfolioTotal: includeInPortfolioTotal,
            totalBalanceUSD: totalBalance,
            addresses: makeAddressSnapshots(),
...
extension TransactionRecord: PlatformSnapshotConvertible {
    func makePlatformSnapshot() -> PlatformTransactionSnapshot {
        ...
        return PlatformTransactionSnapshot(
            ...
            amount: amount,
            address: address,
            transactionHash: transactionHash?.platformTrimmedOrNil,
            failureReason: failureReason?.platformTrimmedOrNil,
            transactionHistorySource: transactionHistorySource?.platformTrimmedOrNil,
            createdAt: createdAt
        )
    }
}
    ...
    func makePlatformSnapshotEnvelope(generatedAt: Date = Date()) -> PlatformSnapshotEnvelope {
        PlatformSnapshotEnvelope(
            ...
            app: PlatformAppSnapshot(
                ...
                fiatCurrency: selectedFiatCurrency.rawValue,
                walletCount: wallets.count,
                transactionCount: transactions.count,
                addressBookCount: addressBook.count,
                wallets: wallets.map { $0.makePlatformSnapshot() },
                portfolio: cachedPortfolio.map { $0.makePlatformSnapshot() },
                transactions: transactions.map { $0.makePlatformSnapshot() },
                addressBook: addressBook.map { $0.makePlatformSnapshot() },
